...
public final class HtmlPage {
    ...
    public static void render(HttpServletResponse response, String title, String body, String backUrl) throws IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<style>");
            out.println("body{background:#f8f9fa}.card{border-radius:1rem}.table td,.table th{vertical-align:middle}");
            out.println(".no-print{display:none!important}");
            out.println("@media print{");
            out.println("  @page{margin:15mm; size:A4;}");
            out.println("  .no-print{display:none!important}");
            out.println("  body{background:#fff!important;}");
            out.println("  .card{box-shadow:none!important;border:1px solid #ccc!important;}");
            out.println("  .btn{display:none!important}");
            out.println("}");
            out.println("</style>");
            out.println("</head>");
            out.println("<body>");
            out.println("<div class=\"container py-4\">");
            out.println("<div class=\"d-flex justify-content-between align-items-center mb-4 no-print\">");
            out.println("<h1 class=\"h3 mb-0\">" + esc(title) + "</h1>");
            if (backUrl != null) {
                out.println("<a class=\"btn btn-outline-secondary\" href=\"" + esc(backUrl) + "\"><i class=\"bi bi-arrow-left me-1\"></i>Back</a>");
            }
            out.println("</div>");
            out.println(body);
            out.println("</div>");
